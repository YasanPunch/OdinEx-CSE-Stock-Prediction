...
class LSTMPredictor(BaseStockModel):
    def __init__(self):
    ...
    def get_model_info(self) -> Dict[str, Any]:
        return {
            'name': 'Bidirectional LSTM Ensemble',
            'description': 'Multi-model ensemble using bidirectional LSTM for time series prediction',
            'strengths': [
                'Strong capability for capturing long-term dependencies',
                'Better memory retention compared to GRU',
                'Ensemble approach reduces prediction variance',
                'Bidirectional architecture captures both past and future context'
            ],
            'limitations': [
                'Requires substantial training data',
                'Higher computational complexity than GRU',
                'May overfit on small datasets',
                'More parameters to train compared to simpler models'
            ]
        }
    ...
    def prepare_data(self, data: np.ndarray, features: List[str]) -> Tuple[np.ndarray, np.ndarray]:
        ...
        try:
            ...
            # Ensure 3D tensor for LSTM input [batch, sequence, features]
            if len(X.shape) == 1:
