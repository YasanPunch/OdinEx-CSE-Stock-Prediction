...
BACKGROUND_TRAINING_RESULT = None

# Application state management
class AppState:
    """
    Streamlined state management using Streamlit session_state directly
    without redundant duplication of state
    """
    ...
    def initialize():
        ...
        # Model instance
        if 'model_instance' not in st.session_state and st.session_state.get('model_type'):
            try:
                st.session_state.model_instance = get_model_class(st.session_state.model_type)()
            except Exception as e:
                st.session_state.error_message = f"Error initializing model: {str(e)}"
        ...
        if 'tech_chart_type' not in st.session_state:
            st.session_state.tech_chart_type = "Line"
            
        if 'tech_date_range' not in st.session_state:
            st.session_state.tech_date_range = "6 Months"
            
        if 'tech_indicator' not in st.session_state:
            st.session_state.tech_indicator = "RSI"
    
    @staticmethod
    def update_model_type(new_model_type):
        """Update model type and reinitialize model instance if needed"""
        if new_model_type != st.session_state.get('model_type'):
            st.session_state.model_type = new_model_type
            try:
                st.session_state.model_instance = get_model_class(new_model_type)()
            except Exception as e:
                st.session_state.error_message = f"Error initializing model: {str(e)}"
                st.session_state.model_instance = None
    
    @staticmethod
    def update_company(new_company):
        """Update selected company and load data info"""
        if new_company != st.session_state.get('selected_company'):
            st.session_state.selected_company = new_company
            
            # Update data info for the company
            try:
                st.session_state.data_info = data_processor.get_dataset_statistics(new_company)
            except Exception as e:
                st.session_state.error_message = f"Error loading company data: {str(e)}"
                st.session_state.data_info = {}
    ...
    @staticmethod
    def add_to_comparison(model_data):
        """Add model to comparison list if not already present"""
    ...
    @staticmethod
    def remove_last_comparison():
        """Remove the last model from comparison"""
        if st.session_state.comparison_models:
            st.session_state.comparison_models.pop()
            return True
        return False

def show_error_message():
    """Display error message from session state if present and provide clear button"""
...
# Function to train a model (for background processing)
...
def get_or_train_model(model_type: str, company: str, parameters: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    try:
        logger.info(f"Training request: Model={model_type}, Company={company}, Params={parameters}")
        ...
        if cache_file.exists():
            logger.info(f"Found cached model: {cache_file}")
            try:
                ...
                logger.warning(f"Cache load failed: {str(e)}. Training new model...")
        ...
        train_data, test_data = data_processor.get_train_test_split(X, test_size=30)
        train_labels, test_labels = data_processor.get_train_test_split(y, test_size=30)
        ...
        progress_values = {"value": 0, "message": "Starting training..."}
        
        def update_progress(progress, message):
            progress_values["value"] = progress
            progress_values["message"] = message
        
        # Set progress callback if model supports it
        if hasattr(model_instance, 'set_progress_callback'):
            model_instance.set_progress_callback(update_progress)
        
        # Train model
        logger.info(f"Starting model training with {len(train_data)} training samples")
        ...
        try:
            ...
            logger.info(f"Generated predictions for {prediction_days} days")
...
def train_model_background(model_type, company, parameters):
    """
    Training function designed for Streamlit without background threads.
    Uses session state to track progress and results.
    """
    try:
        # Initialize training state
        st.session_state.training_in_progress = True
        st.session_state.training_progress = 0
        st.session_state.training_status = "Preparing training data..."
        
        # This will be displayed in the UI
        progress_container = st.empty()
        status_container = st.empty()
        
        # Create a custom callback that updates session state and UI
        ...
        if not validate_data(df):
            progress_callback(0, "Data validation failed")
            st.session_state.training_in_progress = False
            st.session_state.error_message = "Data validation failed. Please check the logs."
            return
            
        progress_callback(0.1, "Preparing features...")
        ...
        try:
            ...
            progress_callback(0, f"Failed to create sequences: {str(e)}")
            st.session_state.training_in_progress = False
            st.session_state.error_message = f"Failed to create sequences: {str(e)}"
            return
            
        progress_callback(0.2, "Splitting data into train/test sets...")
        
        # Split data for training and testing
        train_data, test_data = data_processor.get_train_test_split(X, test_size=30)
        train_labels, test_labels = data_processor.get_train_test_split(y, test_size=30)
        
        progress_callback(0.3, "Initializing model...")
        ...
        if hasattr(model_instance, 'set_progress_callback'):
            def model_progress_callback(progress, message):
                # Scale progress to the 0.3-0.8 range
                scaled_progress = 0.3 + (progress * 0.5)
                progress_callback(scaled_progress, message)
                
            model_instance.set_progress_callback(model_progress_callback)
        
        progress_callback(0.3, f"Training model with {len(train_data)} samples...")
        ...
        progress_callback(0.8, "Generating predictions...")
        ...
        time.sleep(1)  # Give the user a moment to see the completion
        st.rerun()
        ...
        st.session_state.error_message = f"Training error: {str(e)}"
...
def check_background_training_result():
    """Check if background training has completed and update session state"""
    
    if BACKGROUND_TRAINING_RESULT is not None and BACKGROUND_TRAINING_RESULT.get('completed', False):
        # Training has completed, update session state from the main thread
        result = BACKGROUND_TRAINING_RESULT
        BACKGROUND_TRAINING_RESULT = None  # Reset for next training
        
        # Update session state
        st.session_state.training_in_progress = False
        st.session_state.training_completed = True
        st.session_state.training_success = result.get('success', False)
        
        if result.get('success', False):
            # Store model data
            st.session_state.current_model = result.get('model_data', {})
            st.session_state.last_trained = result.get('timestamp', datetime.now())
            ...
            history_entry = {
                ...
                'metrics': result.get('model_data', {}).get('metrics', {})
            }
            
            st.session_state.training_history.append(history_entry)
        else:
            # Store error message
            st.session_state.error_message = result.get('error_message', "Unknown training error")
...
def main():
    ...
    with st.sidebar:
        ...
        if model_type != st.session_state.model_type:
            ...
            AppState.model_type = model_type
            AppState.model_instance = st.session_state.model_instance
        ...
        AppState.current_params = current_params
        ...
        # Training Button
        if st.button("Train Model", disabled=st.session_state.get('training_in_progress', False)):
            if not companies:
                ...
                # Use the direct training approach instead of background threading
        ...
        # Remove the progress simulation loop entirely - the new function handles progress updates
    ...
    # Prediction Tab
    with tabs[0]:
        AppState.active_tab = "Prediction"
            ...
            try:
                ...
                with col1:
                    ...
                    try:
                        ...
                        # Create historical performance plot using helper
                        fig1 = create_performance_plot(
                            actual_dates,
                            actual_prices,
                            predicted_prices,
                            title=f'Historical Price Comparison - {company}'
                        )
                        ...
                        try:
                            if len(future_predictions) > 0 and len(actual_prices) > 0:
                                ...
                                with pred_cols[0]:
                                    st.metric(
                                        "Last Known Price", 
                                        f"LKR {actual_prices[-1]:.2f}"
                                    )
                                    
                                with pred_cols[1]:
                                    st.metric(
                                        "Predicted End Price", 
                                        f"LKR {future_predictions[-1]:.2f}",
                                        f"{price_change:.2f}%",
                                ...
                                with pred_cols[2]:
                                    st.metric(
                                        "Predicted Trend", 
                                        trend_data['trend'],
                                        f"{trend_data['strength']:.1f}% confidence"
                                    )
                    ...
                    with info_cols[1]:
                        ...
                        with control_cols[0]:
                            if st.button("Add to Comparison"):
                                if model_data not in st.session_state.comparison_models:
                                    st.session_state.comparison_models.append(model_data)
                                    AppState.comparison_models = st.session_state.comparison_models
    ...
    with tabs[1]:
        AppState.active_tab = "Model Comparison"
    ...
    # Add this to the beginning of the technical analysis tab
    with tabs[2]:
        AppState.active_tab = "Technical Analysis"
        
        st.markdown("## Technical Analysis")
        
        # Initialize technical analysis state if needed
        if 'tech_chart_type' not in st.session_state:
            st.session_state.tech_chart_type = "Line"
        if 'tech_date_range' not in st.session_state:
            st.session_state.tech_date_range = "6 Months"
    ...
    # Data Explorer Tab
    with tabs[3]:
        AppState.active_tab = "Data Explorer"
